...
public class GetMyOutbidProductsQueryHandler(IMongoDatabase database) : IRequestHandler<GetMyOutbidProductsQuery, PaginatedList<ProductResponse>>
{
    private readonly IMongoCollection<Product> _products = database.GetCollection<Product>("Products");

    public async Task<PaginatedList<ProductResponse>> Handle(GetMyOutbidProductsQuery request, CancellationToken cancellationToken)
    {
        var filter = Builders<Product>.Filter.ElemMatch(
            p => p.Listing!.Auction!.Bids,
            b => b.BidderId == request.UserId && b.Status == "Outbid");
