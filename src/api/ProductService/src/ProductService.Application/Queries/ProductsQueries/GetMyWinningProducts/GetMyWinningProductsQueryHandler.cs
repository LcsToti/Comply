...
namespace ProductService.Application.Queries.ProductsQueries.GetMyWinningProducts;

public class GetMyWinningProductsQueryHandler(IMongoDatabase database) : IRequestHandler<GetMyWinningProductsQuery, PaginatedList<ProductResponse>>
{
    private readonly IMongoCollection<Product> _products = database.GetCollection<Product>("Products");

    public async Task<PaginatedList<ProductResponse>> Handle(GetMyWinningProductsQuery request, CancellationToken cancellationToken)
    {
        var filter = Builders<Product>.Filter.ElemMatch(
            p => p.Listing!.Auction!.Bids,
            b => b.BidderId == request.UserId && b.Status == "Winning");
