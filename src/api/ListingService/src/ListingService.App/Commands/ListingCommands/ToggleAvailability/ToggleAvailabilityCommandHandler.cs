...
public class ToggleAvailabilityCommandHandler(
        ...
        ILogger<ToggleAvailabilityCommandHandler> logger) : IRequestHandler<ToggleAvailabilityCommand, Result<ListingResult>>
{
    ...
    private readonly ILogger<ToggleAvailabilityCommandHandler> _logger = logger;
    private readonly IDateTimeProvider _dateTimeProvider = dateTimeProvider;

    public async Task<Result<ListingResult>> Handle(ToggleAvailabilityCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Handling ToggleAvailabilityCommand for Listing {ListingId}", request.ListingId);

        // App Logic
        var listing = await _listingRepository.GetByIdAsync(request.ListingId);

        if (listing is null) return Result<ListingResult>.Failure(new NotFound(nameof(Listing), request.ListingId));

        if (listing.SellerId != request.UserId)
            return Result<ListingResult>.Failure(new Forbidden("It is not possible to toggle the availability of someone else's listing."));

        // Domain
        listing.ToggleAvailability(_dateTimeProvider.UtcNow);

        // Persist
        await _repositoryCommandsOrchestrator.UpdateListingAsync(listing, cancellationToken);

        // Finish
        _logger.LogInformation("Listing {Id} toggled it's availability", listing.Id);
